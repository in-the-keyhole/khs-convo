...
var drawing = function(session, request, even, data) {
...
var grade = function (session, request, event, data) {
    var compiledFunction = pug.compileFile(template_dir + '/quiz/quiz.pug');

    var questionsLength = quiz.length;
    var correctCount = 0;
    var result = [];
    ...
    var html = compiledFunction({ url: host_url, results: result, score: (correctCount+"/"+questionsLength) });
    var word = event.words[0].word;
    var result = "You got " + correctCount + " / " + questionsLength + " questions correct.\n Text your first and last name to be entered in a $200 Amazon gift card Drawing\n";
    result += 'LINK to Quiz Results and Explanations-->\n' + host_url + 'api/public/html/' + request.phone + '/' + word;
    mongo.Update({ phone: request.phone, event: word }, { phone: request.phone, event: word, html: html }, "ui", { upsert: true });
