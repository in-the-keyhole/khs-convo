...
module.exports = function (events) {
    var event = {};
    event.isAuth = false;
    event.description = "Notify Jenkins Build Failure by Slack";
    event.words = [{ word: 'slackfail', value: 10 }];
    event.run = function (request) {

        return new Promise(function (resolve, reject) {

            slackService.getSlackInfo(false).then(function (result) {
                ...
                khslack.webhook({
                    ...
                    if (err) {
                        ...
                        if (response.response.indexOf('Bad token')!=-1){
                            ...
                            return resolve(slackInfo.slackMessage);
                        }
                    }
                })
            })
